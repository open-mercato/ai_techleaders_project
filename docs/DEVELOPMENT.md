# 🛠️ DevMentor — development guide

Everything you need to run, configure, test and understand DevMentor locally. The
[README](../README.md) covers what the product does; this page covers how it works.

> Working in this repository as a human or an AI agent? Read
> **[AGENTS.md](../AGENTS.md)** as well — it documents the conventions and the gotchas
> that cost us time the first way around.
...
| `npm run build-storybook` | Build the prototype and the static component catalogue |
| `npm run prototype` | Rebuild the clickable journey preview from `prototypes/devmentor-journey/` |
...
No route hand-rolls fetch, validation, authorization or error handling:

- `packages/core/src/http/` — the typed `AppError` hierarchy, `apiHandler` (maps
  errors to a `{ ok, data }` / `{ ok, error }` envelope and requires the CSRF header on
  every mutating verb), `makeCrudRoute` (schema + service in, `{ GET, POST, PUT, DELETE }`
  out) and the `requireSession` / `requireRole` guards.
- `packages/ui/src/backend/` — `apiCall` (the only sanctioned `fetch()` call site),
  `CrudForm`, `DataTable`, and the loading/error/empty feedback components.
...
- **Public / marketing pages** (`/`, `/m/<slug>`, `/invitation/<token>`, `(auth)/*`)
  use plain Tailwind CSS v4 utilities.
- **Signed-in surfaces** (`(mentee)/*`, `(mentor)/*`, `/admin/*`) use shadcn-ui
  components from `@devmentor/ui` inside `AppShell`. The design tokens live in
  `packages/ui/src/tokens.css`; add components with `npx shadcn@latest add <component>`
  run inside `packages/ui`.

### Spec-driven development

Nontrivial work is specified before it is built. Specs, autonomous-run logs, and
lessons live under `.ai/` — see [AGENTS.md](../AGENTS.md) for the full workflow and
[SDLC.md](../SDLC.md) for how a change travels from ticket to merge.
