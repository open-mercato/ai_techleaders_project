...
| When | Two `POST /api/invitations/<token>/accept` requests are sent at the same time with the same cookie | Exactly one answers `200` `{ ok: true, data: { roles: ["mentee", "mentor"], … } }`. The other answers with a status other than 200 and `ok: false`. |
