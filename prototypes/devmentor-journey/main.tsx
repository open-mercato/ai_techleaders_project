...
function Prototype() {
 ...
 const [confirmed,setConfirmed]=useState<{slot:Slot;duration:25|50;price:number;ownerId:string}|null>(null);
 ...
 const auth=useAuthController(selected!==null,confirmed!==null && confirmed.ownerId===authDemo.getSession()?.id);
 ...
 function outcome(value:'confirmed'|'failed'|'slot-taken'|'expired') {
   if(value==='confirmed') {
     if(selected && auth.user) setConfirmed({slot:selected,duration,price:prices[duration],ownerId:auth.user.id});
     navigate('s6');return;
   }
 ...
 return <AuthContext.Provider value={auth}>
  ...
  <div className="doc"><header className="doc-head"><p className="proto-eyebrow">DevMentor Design System: clickable flow</p><h1>Accounts, booking and mentor sessions</h1><p>This demo uses fictional data and simulates sign-in and payments locally. Demo clock: 10 September 2026, 08:00 UTC. Review comments stay in this browser until exported.</p></header>
   ...
   <BookingScreens slots={slots} selected={selected} duration={duration} prices={prices} timeZone={timeZone} signedIn={auth.user!==null} onSelect={setSelectedId} onDuration={setDuration} onTimeZone={setTimeZone} onOutcome={outcome} conflict={conflict}/>
   <SessionScreens hasBooking={auth.hasSession} duration={confirmed?.duration??25} sessionPrice={confirmed?.price??180} slots={slots} startsAt={session.start} dateLabel={dateLabel(session.start,timeZone)} timeLabel={timeLabel(session.start,timeZone)} timeZone={timeZone} prices={prices} submittedReview={submittedReview} onReviewSubmit={submitMentorReview} onPricesChange={setPrices} onSlotAdded={startsAt=>setSlots(current=>current.some(s=>s.start===startsAt)?current:[...current,{id:`added-${startsAt}`,start:startsAt}])}/>
  </div>
 </AuthContext.Provider>;
}

const worker=setupWorker(...authHandlers,http.post('/prototype-api/devmentor-journey/save',async({request})=>{
