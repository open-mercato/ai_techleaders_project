...
export function AuthScreens() {
  ...
  return <>
    <Screen id="s12" title="Sign in" description="Email and GitHub sign-in with role-aware continuation and recovery." refs={['E01', '#12', '#13', '#14']}>
      <AuthLayout title={auth.returnTo === 's4' ? 'Sign in to book your session' : 'Welcome back'} description="Sign in to see your sessions, written answers and private notes." footer={<Button size="sm" intent="neutral" appearance="ghost" onClick={() => navigate(auth.returnTo === 's4' ? 's3' : 's17')}>{auth.returnTo === 's4' ? 'Back to your selected time' : 'Back to home'}</Button>}>
    ...
    <Screen id="s22" title="Email verification" description="Valid links create a session; expired and invalid links offer a way to start again." refs={['E01', '#13']}>
      <AuthLayout title={auth.notice === 'verified' ? 'Your email is verified' : 'Check your verification link'} description={auth.notice === 'verified' ? 'Your account is ready.' : 'You can request another link using the same email address.'}>
        <div className="proto-stack"><AuthFeedback state={auth.notice === 'verified' ? 'verified' : auth.notice === 'verification-expired' ? 'verification-expired' : auth.notice === 'service-unavailable' || auth.notice === 'rate-limited' ? auth.notice : 'verification-invalid'}/>{auth.notice === 'verified' ? <Button className="proto-full-button" onClick={auth.complete}>{auth.returnTo === 's4' ? 'Continue with your booking' : 'Open my workspace'}</Button> : <Button className="proto-full-button" onClick={retryRegistration}>Request a new verification link</Button>}<Button intent="neutral" appearance="ghost" onClick={() => auth.switchMode('s12')}>Back to sign in</Button></div>
      </AuthLayout>
    </Screen>
    ...
    <Screen id="s25" title="Operator users" description="Operator-only directory demonstrates access to a protected admin page." refs={['E01', '#14']}><Workspace><div className="proto-stack"><div className="proto-page-heading"><h1>Users</h1><p>Fictional records for testing account access. Role editing is outside this prototype.</p></div><DataTable caption="Demo users" getRowId={row => row.id} rows={DEMO_ACCOUNTS.filter(account => account.id !== 'new-github')} columns={[{key:'displayName',header:'Name'},{key:'email',header:'Email'},{key:'roles',header:'Roles',render:row => <span className="dm-meta-group">{(row.id === 'sam' && auth.user?.id === 'sam' ? auth.user.roles : row.roles).map(role => <span className="dm-meta-chip" key={role}>{role}</span>)}</span>}]}/></div></Workspace></Screen>
  </>;
}
