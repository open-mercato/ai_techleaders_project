...
(function () {
  ...
  function screenFor(screenId) {
    var target = typeof screenId === 'string' && document.getElementById(screenId);
    return target && target.matches('.screen') && target.querySelector('.frame') ? target : null;
  }

  function hashScreenId(hash) {
    try { return decodeURIComponent(hash.slice(1)); }
    catch { return ''; }
  }

  function goTo(screenId, record) {
    var target = screenFor(screenId);
    if (!target) {
    ...
    window.history.replaceState(window.history.state, '', '#' + encodeURIComponent(screenId));
    updateBackButton();
    document.dispatchEvent(new CustomEvent('devmentor:screen-change', { detail: screenId }));
    var heading = target.querySelector('.frame h1, .frame h2');
    if (heading) {
  ...
  function isEditingTarget(target) {
    ...
    if (target.closest('input, textarea, select') || target.isContentEditable) return true;
  ...
  function labelFor(node) {
    ...
    if (!text && node.labels && node.labels[0]) {
      text = (node.labels[0].textContent || '').replace(/\s+/g, ' ').trim();
    }
  ...
  function buildPanel() {
    ...
    authorInput = element('input', {
      ...
      'aria-label': 'Your name for review comments',
      value: author || '',
      oninput: function (event) { author = event.target.value; save(); }
    });
    panel = element('aside', { id: 'devmentor-review-panel', class: 'anno-panel', 'aria-label': 'Prototype comments' }, [
  ...
  function openPanel() {
    ...
    if (threadsButton) threadsButton.setAttribute('aria-expanded', 'true');
  ...
  function closePanel() {
    ...
    if (threadsButton) threadsButton.setAttribute('aria-expanded', 'false');
  ...
  function composerCard() {
    var input = element('textarea', { placeholder: 'What should change?', 'aria-label': 'New review comment' });
  ...
  function threadCard(thread) {
    ...
    if (thread.id === activeId) {
      var reply = element('textarea', { placeholder: 'Reply…', 'aria-label': 'Reply to this review comment' });
  ...
  function setMode(nextMode) {
    ...
    if (clickModeButton) clickModeButton.setAttribute('aria-pressed', String(mode === 'click'));
    if (commentModeButton) commentModeButton.setAttribute('aria-pressed', String(mode === 'comment'));
    if (mode === 'comment' && panel && !reanchorThreadId) openPanel();
  }

  function onDocumentClick(event) {
    if (event.target.closest('.anno-panel')) return;
    if (event.target.closest('.anno-pin')) return;
    var frame = event.target.closest('.frame');

    if (mode === 'comment' && frame && !event.target.closest('.doc-toolbar')) {
      event.preventDefault();
      event.stopPropagation();
    ...
    var screenId = link && hashScreenId(link.getAttribute('href'));
    if (screenFor(screenId)) {
      event.preventDefault();
      goTo(screenId);
    }
  }
  ...
  function buildToolbar() {
    ...
    var segmented = element('div', { class: 'segmented', role: 'group', 'aria-label': 'Prototype review mode' }, [clickModeButton, commentModeButton]);
    var hintsButton = element('button', { class: 'btn btn-outline btn-sm', type: 'button', 'aria-pressed': 'false', text: 'Show hotspots', onclick: function () {
      hints = !hints;
      document.body.classList.toggle('proto-hints', hints);
      hintsButton.textContent = hints ? 'Hide hotspots' : 'Show hotspots';
      hintsButton.setAttribute('aria-pressed', String(hints));
    } });
    var focusButton = element('button', { class: 'btn btn-sm ' + (focusMode ? 'btn-primary' : 'btn-outline'), type: 'button', 'aria-pressed': String(focusMode), text: 'Presentation', onclick: function () {
      focusMode = !focusMode;
    ...
    threadsButton = element('button', { class: 'btn btn-outline btn-sm', type: 'button', 'aria-expanded': 'false', 'aria-controls': 'devmentor-review-panel', text: 'Threads', onclick: function () {
      if (panel.classList.contains('open')) closePanel();
      else openPanel();
    } });
    var pinsButton = element('button', { class: 'btn btn-outline btn-sm', type: 'button', 'aria-pressed': 'true', text: 'Hide pins', onclick: function () {
      pinsVisible = !pinsVisible;
      document.body.classList.toggle('anno-hidden', !pinsVisible);
      pinsButton.textContent = pinsVisible ? 'Hide pins' : 'Show pins';
      pinsButton.setAttribute('aria-pressed', String(pinsVisible));
    } });
  ...
  function start() {
    ...
    var initialScreen = screenFor(hashScreenId(window.location.hash))
      || screenFor(document.documentElement.getAttribute('data-start-screen'))
      || screenFor('s1');
    if (!initialScreen || !document.querySelector('.doc-toolbar')) return false;
    started = true;
    prototypeId = document.documentElement.getAttribute('data-prototype-id') || 'prototype';
    threadsStorageKey = 'om-prototype-comments:v2:' + prototypeId;
    var committedDocument = normalizeDocument(window.__OM_PROTOTYPE_COMMENTS__);
    committedOperations = clone(committedDocument.operations);
    ...
    localOperations = normalizeOperations(load(threadsStorageKey, { version: 2, operations: [] }).operations)
      .filter(function (operation) { return !committedOperationIds.has(operation.id); });
    author = load(authorStorageKey, '');
    focusMode = true;
    document.body.classList.add('proto-focus');
    applyOperations();
    buildPanel();
    buildToolbar();
    buildBackButton();
    ...
    window.addEventListener('keydown', function (event) {
      ...
      if (event.key === 'Backspace' && focusMode && !event.defaultPrevented && !isEditingTarget(event.target)) {
        event.preventDefault();
        goBack();
      }
    });
    goTo(initialScreen.id, false);
    return true;
  }

  // React owns screen rendering; call this after its permanent screen sections mount.
  window.__DEVMENTOR_PROTOTYPE_START__ = start;
})();
