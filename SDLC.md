...
| Triage | Confirm the issue is real, still unfixed on `master`, and not already claimed or covered by an open PR. Read-only; stops the chain cleanly when there is nothing to do. | `om-verify-in-repo` or a human | Confirmed actionable, or closed as no-action |
...
| PR | Commit, push, and open a PR against `master` with normalized labels. On a hand-worked branch, `om-check-and-commit` runs the gate, fixes obvious drift, and pushes when green. | `om-open-pr`, `om-auto-create-pr`, or `om-check-and-commit` | Open, labeled PR |
...
| Merge | `om-merge-buddy` reports, read-only, which PRs can merge now and which are close but blocked. `om-approve-merge-pr` re-checks every gate, approves, and squash-merges. | `om-merge-buddy` + `om-approve-merge-pr`, or a human | PR squash-merged into `master` |
...
In this repository the browser side of QA runs through the `agent-browser` provider described in `.ai/browsers/agent-browser.md`; the repository-native integration suite (`npm run test:integration`, Testcontainers + a real production build) remains the authoritative automated check and is not replaced by ad-hoc browser exploration.
...
This repository already practices Spec-Driven Development under `.ai/` (see `AGENTS.md`): specs live in `.ai/specs/` and graduate to `.ai/specs/implemented/`, autonomous runs log to `.ai/runs/`, and lessons land in `.ai/lessons.md`. The pipeline skills use the same directories — `paths.specs`, `paths.runs` — so spec-writing (`om-spec-writing`, `om-auto-write-spec`) and implementation (`om-auto-implement-spec`) slot into the existing convention rather than introducing a parallel one.
...
`npm run test:unit:coverage` is deliberately the test command rather than plain `npm run test:unit`: this project requires 100% statement, branch, function, and line coverage per file for every new or changed production file, and the coverage run is what enforces it. Every such file must also be added to `coverage.include` in `vitest.config.mts` in the same change.

The gate omits `npm run test:integration` because it provisions an ephemeral PostgreSQL through Testcontainers, builds and boots the app, and drives a real browser — minutes of work and a Docker dependency that does not belong in every local pre-commit loop. It is not optional, only relocated: GitHub Actions runs it on every pull request as the `Integration tests` check, and `om-integration-tests` runs it on demand when a change crosses the app/database/API/browser boundary.

CI runs Build, Lint, Unit tests, and Integration tests independently on every pull request. Those results become merge-blocking only when the repository ruleset requires those four exact check names.
